...
contract BaseCustomCurveTest is Test, Deployers {
    ...
    function test_addLiquidity_swapThenAdd_succeeds() public {
        ...
        hook.addLiquidity(
            BaseCustomAccounting.AddLiquidityParams(
                5 ether, 5 ether, 4 ether, 4 ether, address(this), MAX_DEADLINE, MIN_TICK, MAX_TICK, bytes32(0)
            )
        );
    ...
    function test_addLiquidity_expired_revert() public {
        ...
        hook.addLiquidity(
            BaseCustomAccounting.AddLiquidityParams(
                0, 0, 0, 0, address(this), block.timestamp - 1, MIN_TICK, MAX_TICK, bytes32(0)
            )
        );
    }
    ...
    function test_swap_twoSwaps_succeeds() public {
        hook.addLiquidity(
            BaseCustomAccounting.AddLiquidityParams(
                2 ether, 2 ether, 0, 0, address(this), MAX_DEADLINE, MIN_TICK, MAX_TICK, bytes32(0)
            )
        );
    ...
    function test_removeLiquidity_initialRemove_succeeds() public {
        hook.addLiquidity(
            BaseCustomAccounting.AddLiquidityParams(
                100 ether, 100 ether, 99 ether, 99 ether, address(this), MAX_DEADLINE, MIN_TICK, MAX_TICK, bytes32(0)
            )
        );
        ...
        BaseCustomAccounting.RemoveLiquidityParams memory removeLiquidityParams =
            BaseCustomAccounting.RemoveLiquidityParams(1 ether, MAX_DEADLINE, MIN_TICK, MAX_TICK, bytes32(0));
    ...
    function test_removeLiquidity_fuzz_succeeds(uint256 amount) public {
        ...
        if (amount > hook.balanceOf(address(this))) {
            ...
            hook.removeLiquidity(
                BaseCustomAccounting.RemoveLiquidityParams(amount, MAX_DEADLINE, MIN_TICK, MAX_TICK, bytes32(0))
            );
        } else {
    ...
    function test_removeLiquidity_noLiquidity_reverts() public {
        vm.expectRevert();
        hook.removeLiquidity(
            BaseCustomAccounting.RemoveLiquidityParams(1000000 ether, MAX_DEADLINE, MIN_TICK, MAX_TICK, bytes32(0))
        );
    }
    ...
    function test_removeLiquidity_partial_succeeds() public {
        ...
        hook.removeLiquidity(
            BaseCustomAccounting.RemoveLiquidityParams(5 ether, MAX_DEADLINE, MIN_TICK, MAX_TICK, bytes32(0))
        );
    ...
    function test_removeLiquidity_allFuzz_succeeds(uint112 amount) public {
        ...
        hook.removeLiquidity(
            BaseCustomAccounting.RemoveLiquidityParams(liquidityTokenBal, MAX_DEADLINE, MIN_TICK, MAX_TICK, bytes32(0))
        );
    ...
    function test_removeLiquidity_multiple_succeeds() public {
        ...
        hook.removeLiquidity(
            BaseCustomAccounting.RemoveLiquidityParams(
                hook.balanceOf(address(this)), MAX_DEADLINE, MIN_TICK, MAX_TICK, bytes32(0)
            )
        );
    ...
    function test_addLiquidity_notInitialized_reverts() public {
        ...
        uninitializedHook.addLiquidity(
            BaseCustomAccounting.AddLiquidityParams(
                1 ether, 1 ether, 0, 0, address(this), MAX_DEADLINE, MIN_TICK, MAX_TICK, bytes32(0)
            )
        );
    }
    ...
    function test_swap_addThenRemove_succeeds() public {
        ...
        hook.addLiquidity(
            BaseCustomAccounting.AddLiquidityParams(
                0, 1 ether, 0, 0, address(this), MAX_DEADLINE, MIN_TICK, MAX_TICK, bytes32(0)
            )
        );
        ...
        IPoolManager.SwapParams memory params =
            IPoolManager.SwapParams({zeroForOne: true, amountSpecified: -0.5 ether, sqrtPriceLimitX96: SQRT_PRICE_1_2});
        PoolSwapTest.TestSettings memory settings =
            PoolSwapTest.TestSettings({takeClaims: false, settleUsingBurn: false});
