...
import {AntiSandwichHookMsgSenderMock} from "../mocks/AntiSandwichHookMsgSenderMock.sol";
...
contract AntiSandwichHookNoMsgSenderTest is Test, Deployers {
    AntiSandwichHookMsgSenderMock hook;
    PoolKey noHookKey;
    ...
    function setUp() public {
        ...
        v4Router = new MockV4Router(address(manager));

        hook = AntiSandwichHookMsgSenderMock(
            address(uint160(Hooks.BEFORE_SWAP_FLAG | Hooks.AFTER_SWAP_FLAG | Hooks.AFTER_SWAP_RETURNS_DELTA_FLAG))
        );
        deployCodeTo("test/mocks/AntiSandwichHookMsgSenderMock.sol:AntiSandwichHookMsgSenderMock", abi.encode(manager), address(hook));

        (key,) = initPoolAndAddLiquidity(
            currency0, currency1, IHooks(address(hook)), LPFeeLibrary.DYNAMIC_FEE_FLAG, SQRT_PRICE_1_1
        );

        hook.addRouter(address(v4Router));
